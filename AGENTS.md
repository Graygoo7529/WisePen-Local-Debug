...
- **Rust 代理层（src-tauri/src/）**：所有后端 HTTP 请求经 reqwest 转发，绕开 WebView CORS。
  ...
  - `sse.rs` — `chat_stream`：统一转发 GET/POST SSE，按帧解析后经 `Channel<StreamFrame>` 推送；`abort_chat` 只中断本地订阅。
...
- **前端（src/）**：
  ...
  - `api/chat.ts`（Python /chat 全端点）、`api/asset.ts`（Java /skill/* /agent/*）、`api/resource.ts`、`api/sse.ts`（新建、重连、恢复三类 Chat SSE）。
  - `lib/types.ts` — 全部 API 类型；`lib/format.ts` 格式化工具；`lib/cn.ts` className 合并。
  - `stores/settingsStore.ts` — 连接与凭据（localStorage 持久化，**凭据绝不写入仓库**）。
  - `stores/chatStore.ts` — 会话/历史/实时回合（SSE 归约、active Turn 重连、后端取消、客户端工具和审批恢复）。
